...
- **Projects** — code/name/budget/dates through an Active → On Hold → Completed lifecycle, manual expense entry and deletion (Labor/Material/Other), a running expense summary against budget, a filterable expense history, and a Crew section for assigning/removing workers.
...
- **Admin** — user list with activate/deactivate and Keycloak realm-role assignment (users without a reachable Keycloak account still open, with role management disabled).
